...
from app.services.documents import create_invoice_document, create_joining_letter_document, new_verification_code
...
def create_joining_letter(student_id: int, db: Session = Depends(get_db), user: User = Depends(require_roles(*STAFF_ROLES))):
    ...
    doc = create_joining_letter_document(db, student, created_by=user.id)
...
def create_invoice(student_id: int, db: Session = Depends(get_db), user: User = Depends(require_roles(*STAFF_ROLES))):
    ...
    student = _get_student_or_404(db, student_id)
    doc = create_invoice_document(db, student, created_by=user.id)
    if doc is None:
